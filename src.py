...
writer = SummaryWriter("runs/training_2")
...
def build_model_and_train():
    ...
    # truth.
    decoder = AttnGruDecoder(input_size=768, hidden_size=1200)
    decoder.init_weights()

    # This line loads weights if they are already present
    if os.path.exists("pre_trained/sciq_weights/encoder"):
        encoder.load_state_dict(torch.load("pre_trained/sciq_weights/encoder"))
    if os.path.exists("pre_trained/sciq_weights/decoder"):
        decoder.load_state_dict(torch.load("pre_trained/sciq_weights/decoder"))
